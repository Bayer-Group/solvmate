...
class SMAbsDataset():
    ...
    def _read_dataframe(self, df:pd.DataFrame):
        ...
        assert df["solvent SMILES"].apply(lambda smi: smi.count(".")).max() <= self.max_mix
        ...
        for i in range(self.max_mix):
            ith_part = df["solvent SMILES"].apply(lambda s: split_ith_else(s,i))
            self.mol_dict_solv.append(
    ...
    def __getitem__(self, idx):
        ...
        for mix_comp_idx in range(self.max_mix):
            g_solv = self._load_graph(self.mol_dict_solv[mix_comp_idx],idx,)
            g_solvs.append(g_solv)

        facs = self.mixture_coefficients[idx]
        ...
        assert len(facs) == 2
        eta = 0.0001
        assert abs(sum(facs) - 1) < eta
        return g_solu, g_solvs[0], g_solvs[1], facs, temp, conc
