...
class COSMOParser(object):
    ...
    def _read_turbomole_atom_section(self, cosmofile):
        ...
        while line:
            ...
            self.cosmo_info["atm_pos"].append([float(val) for val in line_splt[1:4]])
            self.cosmo_info["atm_elmnt"].append(line_splt[4].title())
            self.cosmo_info["atm_rad"].append(float(line_splt[5]))

        self.cosmo_info["atm_nr"] = np.array(self.cosmo_info["atm_nr"], dtype="int64")
        self.cosmo_info["atm_pos"] = (
            np.array(self.cosmo_info["atm_pos"], dtype="float64") * angstrom_per_bohr
        )
    ...
    def _read_turbomole_seg_section(self, cosmofile):
        ...
        while line:
            ...
            self.cosmo_info["seg_pos"].append([float(val) for val in line_splt[2:5]])
        ...
        self.cosmo_info["seg_nr"] = np.array(self.cosmo_info["seg_nr"], dtype="int64")
        self.cosmo_info["seg_atm_nr"] = np.array(
            self.cosmo_info["seg_atm_nr"], dtype="int64"
        )
        self.cosmo_info["seg_pos"] = (
            np.array(self.cosmo_info["seg_pos"], dtype="float64") * angstrom_per_bohr
        )
    ...
    def _read_orca_atom_coordinates(self, cosmofile):
        ...
        while line:
            ...
            self.cosmo_info["atm_pos"].append([float(val) for val in line_splt[1:]])
    ...
    def _read_orca_atom_radii(self, cosmofile):
        ...
        self.cosmo_info["atm_rad"] = (
            np.array(self.cosmo_info["atm_rad"], dtype="float64") * angstrom_per_bohr
        )
    ...
    def _read_orca_seg_section(self, cosmofile):
        ...
        self.cosmo_info["seg_area"] = (
            np.array(self.cosmo_info["seg_area"], dtype="float64")
            * angstrom_per_bohr**2
        )
        self.cosmo_info["seg_sigma_raw"] = (
            self.cosmo_info["seg_charge"] / self.cosmo_info["seg_area"]
        )
        ...
        with open(self.filepath, "r") as cosmofile:
            ...
            for line in cosmofile:
                ...
                if "DIPOLE MOMENT (Debye)" in line:
                    line = next(cosmofile).strip()
                    self.cosmo_info["dipole_moment"] = np.array(
