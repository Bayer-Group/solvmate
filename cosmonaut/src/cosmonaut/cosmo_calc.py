...
from cosmonaut.input_parsers import COSMOParser

kJ_per_kcal = 4.184  # kJ/kcal
kJdivmol_per_hartree = 2625.499639479  # (kJ/mol)/hartree

# The default timeout is 12 hours. We just
# assume that no calculation should take more
# than this. Especially for training, we cannot
# afford to take more than half a day on a single
# calculation.
#
# TODO: in the long run, this should be increased
# TODO: to be about 48h
TIME_OUT = 3600 * 48
...
class ConformerGenerator(object):
    ...
    def get_embedded_mol(smiles):
        ...
        if mol is None:
            raise ValueError(
                "Could not generate mol struct from smiles: {}".format(smiles)
            )
    ...
    def __init__(
        self, name, smiles, charge, dir_job, n_cores=1, max_RAM_per_core_in_MB=None
    ):
        ...
        if self.charge != charge_from_molecule:
            raise ValueError(
                "The charge of following molecule does not agree with the smiles: "
                + self.name
            )
    ...
    def _setup_folder(self):
        assert not os.path.exists(
            self.dir_job
        ), f"internal logic error: directory {self.job_dir} already exists!"
    ...
    def calculate_rdkit(
        self,
        number_of_conformers_generated=None,
        rms_threshold=0.5,
        rms_only_heavy_atoms=False,
    ):
        ...
        print(
            f"- settings used: [number_of_conformers_generated: {number_of_conformers_generated}, rms_threshold: {rms_threshold}, rms_only_heavy_atoms: {rms_only_heavy_atoms}]"
        )
        ...
        _ = Chem.AllChem.EmbedMultipleConfs(
            self.mol, numConfs=number_of_conformers_generated, randomSeed=0xF00D
        )
            ...
            for idx1 in range(idx0 + 1, n_conf):
                ...
                if idx_to_keep[idx0] and idx_to_keep[idx1]:
                    rms = ConformerGenerator.calculate_best_rms(
                        ...
                        idx0,
                        idx1,
                        only_heavy_atoms=rms_only_heavy_atoms,
                    )
    ...
    def calculate_orca(self, method):
        ...
        for conformer_index in range(len(self.mol.GetConformers())):
            ...
            self.save_to_disk(
                conformer_indices_to_output=[conformer_index], output_folder=dir_struct
            )
            ...
            orcacosmo_filepath = os.path.join(
            ...
            self.mol.GetConformer(conformer_index).SetProp(
                "attached_file", orcacosmo_filepath
            )
            self.mol.GetConformer(conformer_index).SetDoubleProp(
                "energy", orcacosmo.cosmo_info["energy_tot"] / kJdivmol_per_hartree
            )

            self._update_conformer_coordinates(
                conformer_index, orcacosmo.cosmo_info["atm_pos"]
            )
    ...
    def sort_by_energy(
        ...
        idx_arr = energies.argsort()
        self._sort(new_conformer_indices=idx_arr[0:max_conformers])
        self.save_to_disk()
        return self.current_step_number
                ...
                for idx1 in range(idx0 + 1, n_conf):
                    ...
                    if idx_to_keep[idx0] and idx_to_keep[idx1]:
                        rms = ConformerGenerator.calculate_best_rms(
                            ...
                            idx0,
                            idx1,
                            only_heavy_atoms=only_heavy_atoms,
                        )
    ...
    def _log_results(self, last=False):
        ...
        try:
            print(
                "- %s, n_confs: %s"
                % (self.step_names[index_to_log], len(self.mol.GetConformers()))
            )
        except:
            pass
...
class ORCA(ABC):
    def __init__(self, method_description, step_name, filename_base="geo_opt"):
        ...
        self._orca_full_path = (
            output.stdout.decode("utf-8").split()[index_to_be_used].strip()
        )

        output2 = spr.run(
            [search_command, "otool_xtb"], capture_output=True, timeout=TIME_OUT
        )
    ...
    def execute(
        self, filepath_inp, dir_step, charge, n_cores=None, max_RAM_per_core_in_MB=None
    ):
    ...
    def _call_orca(self):
        with open("log_output.dat", "w") as out:
            spr.run(
                [self._orca_full_path, "input.dat"],
                stdout=out,
                stderr=out,
                timeout=TIME_OUT,
            )
...
class ORCA_DFT_CPCM(ORCA):
    def __init__(
        self, method_description="DFT_CPCM", step_name="ORCA_DFT_CPCM", cpcm_radii=None
    ):
...
def _run_single_cosmo_calculation(
    smiles: str,
    name: str,
    charge: int,
    outputs_dir: Path,
    ...
    try:
        ...
        target_dir = outputs_dir / name

        if target_dir.exists():
            print(f"skipping {smiles} as its already calculated")
            return

        with ConformerGenerator(
            name, smiles, charge, dir_job=target_dir, n_cores=n_cores_inner
        ) as cg:
            ...
            cg.copy_output(os.path.join(cg.dir_job, target_dir / "COSMO"), ["*.xyz"])

        print(f"finished: {name}")
    except Exception as e:
        if "TimeoutExpired" in str(e): # allowed to happen!
            return
        else:
            raise


from smal.all import else_none
def run_cosmo_calculations(
    ...
    assert len(smiles_list) == len(names_list)
    assert len(smiles_list) == len(charges_list)
    assert outputs_dir.exists()
    assert outputs_dir.is_dir()

    joblib.Parallel(n_jobs=n_cores_outer)(
        joblib.delayed(else_none(_run_single_cosmo_calculation))(
            smiles=smiles,
            name=name,
            charge=charge,
        ...
        for smiles, name, charge in zip(smiles_list, names_list, charges_list)
    )

    print(f"finished {run_cosmo_calculations.__name__}")
