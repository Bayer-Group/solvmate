...
class WindowScreener:
    def __init__(
        ...
        if n_estimators_range is None:
            n_estimators_range = [
                100,  # n_estimators for final retrain!
            ]
    ...
    def train_and_eval_recommenders(
        ...
        perform_butina_cv: bool=False,
        perform_solvent_cv: bool=False,
        nova_as_ood: bool=False,
        save_recommender: bool=False,
        job_name: str=None,
        only_source: str="open_notebook",
    ):
        ...
        down_sample_window_screening = False
        if down_sample_window_screening:
            td = td.sample(len(td) // 10)
            pairs = pairs[pairs["solute SMILES"].isin(td["solute SMILES"].tolist())]
        ...
        for reg in tqdm.tqdm(self.regs):
            ...
            for featurizer in self.featurizers:
                ...
                for to_abs_strat in self.abs_strat_range:
                    ...
                    for delta_log_s in [0.0,0.1,0.5,1,1.5,2.0,]:
                        ...
                        if perform_cv:
                            for cv_test in pairs["cross_fold"].unique():
                                ...
                                td_test = apply_delta_s_window(td_test,delta_log_s)
        ...
        return pd.concat(stats_out)

if __name__ == "__main__":
    ws =  WindowScreener()
    stats = ws.train_and_eval_recommenders()
