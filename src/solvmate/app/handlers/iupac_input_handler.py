...
class IupacInputHandler(BaseHandler):
    @tornado.web.authenticated
    @log_function_call
    def post(self):
        try:
            req = json.loads(self.request.body)
            req_smi_or_iup = req["smiles_or_iupac"]

            mol = Chem.MolFromSmiles(req_smi_or_iup)
            if mol is None:
                mol = Chem.MolFromSmiles(opsin_iupac_to_smiles(req_smi_or_iup))
